...
LogValueModel::LogValueModel() : QAbstractListModel() {}
...
void LogValueModel::setData(OptionalReferenceWrapper<std::vector<LogValue>> properties)
{
    beginResetModel();
    data_ = properties;
    endResetModel();
...
// Get LogValue row specified
OptionalReferenceWrapper<LogValue> LogValueModel::getData(int row) const
{
...
int LogValueModel::rowCount(const QModelIndex &parent) const { return data_ ? data_->get().size() : 0; }
...
bool LogValueModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!data_)
        return false;
...
QVariant LogValueModel::headerData(int section, Qt::Orientation orientation, int role) const
{
