...
{
    // Check network reply
    if (handleRequestErrors(worker, "trying to list data directory"))
    {
...
{
    // Check network reply
    if (handleRequestErrors(worker, "trying to generate journals for directory"))
        return;
...
{
    // Check network reply
    if (handleRequestErrors(worker, "trying to stop run data scan for directory"))
        return;
}
