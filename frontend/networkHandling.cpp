...
bool MainWindow::networkRequestHasError(HttpRequestWorker *worker, const QString &taskDescription)
{
    ...
    {
        statusBar()->showMessage(QString("Network error for source %1").arg(currentJournalSource().name()));
    ...
    {
        statusBar()->showMessage(QString("Response error for source %1").arg(currentJournalSource().name()));
        QMessageBox::warning(
            this, "Response Error",
